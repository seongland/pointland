 ...
 * @todo - reduce sql call by pass row
 */
            ...
            .then(row => {
                ...
                fs.readFile(xmlFilePath, 'utf8', function(err, data) {
                    ...
                    xml2js.parseString(strXML, function(err, parsedJson) {
                        parsedJson.krpano.preview[0].$.url = parsedJson.krpano.preview[0].$.url.replace("images/preview.jpg", previewPrepender)
                        parsedJson.krpano.image[0].level.forEach(level => {
                            level.cube[0].$.url = level.cube[0].$.url.replace("images", tilePrepender)
                        })
...
exports.getPreview = async(req, res, next) => {
    try {
        ...
        getNodeInfo(node_id)
            .then(row => {
                let imagePath = getTileFolder(row.round_path, row.seq) + '/images/preview.jpg'
                imagePath = imagePath.replace(/\//gi, '\\')
                imagePath = NAS + imagePath.split(':')[1]
                let imageBuf = fs.readFileSync(imagePath)
                res.send(imageBuf)
...
function getTileFolder(roundPath, seq){
    if (roundPath.search('backpack') >= 0)
        ...
        return `${roundPath}/${TILE_FOLDER}${seq}_Panorama`
}
