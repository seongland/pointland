...
function matrixTimes([[a, b], [c, d]], [x, y]) {
...
function arcSVG(cx, cy, rx, ry, start, angle, rotate) {
  ...
  let [sX, sY] = vectorAdd(
    matrixTimes(rotMatrix, [rx * COS(start), ry * SIN(start)]),
    [cx, cy]
  )
  let [eX, eY] = vectorAdd(
    matrixTimes(rotMatrix, [rx * COS(start + angle), ry * SIN(start + angle)]),
    [cx, cy]
  )
  let fA = angle > π ? 1 : 0
  let fS = angle > 0 ? 1 : 0
  let path_2wk2r = document.createElementNS(
    'http://www.w3.org/2000/svg',
    'path'
  )
  path_2wk2r.setAttribute(
    ...
    `M ${cx} ${cy} ` +
      ...
      sY +
      ` A ` +
      [rx, ry, (rotate / (2 * π)) * 360, fA, fS, eX, eY].join(' ')
  )
  return path_2wk2r
}
